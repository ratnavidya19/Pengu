...
public class PenguModelAStarSearch {
	...
	public void aStarSearch(GameBoard gameboard) {
		...
		pQueue = new PriorityQueue<Nodes>(100, new NodesComparator());
		pQueue.add(new Nodes(0, new String[]{""}));
	

		//iterate while queue is not empty and there are no fishes in the field
		while (!pQueue.isEmpty() && !(pengu_score == 17)) {
			
		    //pop the node from queue depending on the priority
		...
		//transition function
			...
			gameboard2 = transitionFunction(gameboard2, array_moves);
			 ...
			 {
				 //cost function is the total of cost from the initial board to s and the heuristic function 
				 //that returns an estimate of the cost from s to goal. 
				 int cost_function = pengu_score + heuristic_function(gameboard2.pengu_score);
				 for(int i = 0; i < array_moves.length; i++)
				 {
				 //add cost function and array of valid moves to priority queue
					 pQueue.add(new Nodes(cost_function, array_moves));
				 }
				// pop_result = new String[]{""};
	...
	private GameBoard transitionFunction(GameBoard gameboard, String[] pop_result2) {
		...
		for (int i = 0; i < pop_result2.length; i++) {
			...
			//copy original gameboard to another board
